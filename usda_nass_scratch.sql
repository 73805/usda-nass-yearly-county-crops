...
insert into indigo.crops_ltd
	...
	where commodity_desc in ('CORN', 'COTTON', 'RICE', 'SOYBEANS', 'WHEAT')
	  ...
	  and value not in ('(D)', '(Z)') -- in absence of informed replacement
	  ...
	  -- filter out redundant crop sub types 
	  and not (commodity_desc = 'WHEAT' and class_desc = 'ALL CLASSES')
	  -- filter out atypical units
	  and (
...
-- records within year by crop and measurement
...
-- confirm numbers are convertible (briefly replaced '.' as well...)
select sum(replace(value, ',','')::decimal) from indigo.crops_ltd;

-- preview next table build
